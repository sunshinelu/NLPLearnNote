...
class ChineseSegLSTM(object):
    ...
    def run_training(self):
        ...
        input_data = tf.placeholder(tf.float32,
                                    [self._batch_size, self._max_feature_len, self._window_size * self._embedding_size])
        ...
        logits = self.inference(input_data, seq_len, keep_prob)
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            for i in range(10000000):
                ...
                if i % 100 == 0 and i > 0:
                    ...
                    _, loss_value, true_count = sess.run([train_op, cross_entropy_mean, total_correct],
                                                         feed_dict=feed_dict)
                    total_count = 0
    ...
    def inference(self, input_data, seq_len, keep_prob):
        ''' 向前计算过程描述
        Args:
        ...
        num_hidden = 120
        num_layers = 2
        timestep_size = self._max_feature_len
        ...
        def lstm_cell():
            cell = tf.contrib.rnn.LSTMCell(num_hidden, initializer=tf.random_normal_initializer(stddev=0.1))
            return tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=keep_prob)

        fw_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell() for _ in range(num_layers)], state_is_tuple=True)
        bw_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell() for _ in range(num_layers)], state_is_tuple=True)

        initial_state_fw = fw_cell.zero_state(self._batch_size, tf.float32)
        initial_state_bw = bw_cell.zero_state(self._batch_size, tf.float32)

        inputs = tf.unstack(input_data, timestep_size, 1)
        ...
        outputs = tf.reshape(tf.concat(outputs, 1), [-1, num_hidden * 2])
